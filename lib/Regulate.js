;(function (window) {
    ...
    function breakRanges2(start, end) {
        ...
        if (+start[0] < +end[0]) {
            breakdigit = (1 + (+s[0])) + "";
            breakpoint = breakpointToString(parseInt(breakdigit + (new Array(e.length).join("0")), 10) - 1);
    ...
    Regulate.prototype.range = function (a, b) {
        if (typeof a === "number" && typeof b === "number") {
            this._output += "(" + collapsePowers(rangeToRegexes(a + "", b + "")).join("|") + ")";        
        }
        return this;
    };
